...
func TestValidatedPayload(t *testing.T) {
	...
	for _, test := range tests {
		...
		if test.event != "" {
			req.Header.Set(eventHeader, test.event)
		}
		if test.eventID != "" {
			req.Header.Set(deliveryHeader, test.eventID)
		}

		if got := req.Header.Get(deliveryHeader); got != test.wantEventID {
			t.Errorf("eventID = %q, want %q", got, test.wantEventID)
		}
		if got := req.Header.Get(eventHeader); got != test.wantEvent {
			t.Errorf("event = %q, want %q", got, test.wantEvent)
		}
		got, err := ValidatedPayload(req, secretKey)
		if err != nil {
			if test.wantPayload != "" {
				t.Errorf("ValidatedPayload(%#v): err = %v, want nil", test, err)
			}
			continue
		}
		if string(got) != test.wantPayload {
			t.Errorf("PayloadPayload payload = %q, want %q", got, test.wantPayload)
		}
	}
}
